...
method moves(p: Pawn, sq: Sq, b: Board): seq[Sq] =
  ...
  let diagonals = @[Vec(x: sq.x - 1, y: sq.y + p.color.forward),
                    Vec(x: sq.x + 1, y: sq.y + p.color.forward)]
...
let DIAGONALS = fourDirections(Vec(x: 0, y: 1))
...
let STRAIGHT_LINES = fourDirections(Vec(x: 1, y: 1))
